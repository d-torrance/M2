...
`random(d, R)` returns a random element of degree `d` in the ring `R`. Wanting several at once, in
assorted degrees, means writing a loop around it. This file asks for a `Sequence` form so that
`random((4:1, 3:2), R)` means four random elements of degree 1 and three of degree 2 — and supplies
both the one-line implementation and a transcript of it working.
