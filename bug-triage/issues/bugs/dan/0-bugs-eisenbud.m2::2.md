...
`quotient(I, J)` computes the ideal quotient `I : J`. When `J` is generated by a single variable there
is a far cheaper route than the general algorithm, and `quotient` carries a `Linear` strategy intended
for exactly that case. Eisenbud's request is that `quotient` should spot the case and take that route
by itself. Dan's observation, merged in here, is that asking for the strategy by name does not even
work.

It does not because `quotient`'s `Linear` strategy is a stub, and it is commented out of the hook table, so asking for it
