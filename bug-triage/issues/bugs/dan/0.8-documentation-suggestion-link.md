...
Macaulay2's manual installs as html, one page per documentation node, and a reader who notices a
mistake on a page has no way to say so from it. This file asks for a link at the bottom of every page
— a `mailto:` in its 2009 terms, with a Google Group post and a bug tracker floated as alternatives.
The block of URL-encoded text in the middle is a stray example of `mailto:` syntax pasted in for
reference, spam wording and all; it is not part of the request.
