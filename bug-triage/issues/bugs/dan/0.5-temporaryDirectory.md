...
`temporaryFileName` gives Macaulay2 a path for scratch work, but only a *name*: a package that wants a
scratch **directory** has to create one and cope with the name already being taken. This file asks for
`temporaryDirectory` in Core, supplies its implementation, and asks for a companion that removes such a
directory along with everything in it.
