...
`toField` takes a ring that happens to be a field — typically a quotient such as `QQ[a]/(a^2-2)` — and
tells Macaulay2 to treat it as one, so division works and it can serve as a coefficient ring. This
file collects three related wishes about what else that ought to do: give the result a `Field` class
of its own instead of leaving it looking like a polynomial ring; have `newRing` record the ring it came
from, so promotion works across the family; and, from Kahle in particular, let `gens` still return the
