...
`monomialIdeal` builds an ideal generated by monomials. This file shows it over a ring whose
coefficients are themselves a quotient — `(QQ[w]/(w^4+w^3+w^2+w+1))[x,y]` — returning a second
generator that is not a monomial at all, but a constant. Dan's note is that the answer *"appears
wrong, because of the appearance of the extra constant"*.

He is right, and the consequence is worse than an untidy generating set. `monomialIdeal` picks up a
spurious generator when the coefficient ring is a quotient, and the ideal
