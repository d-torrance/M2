...
| 1 | `bugs/dan/1-TAGS` | &nbsp; | &nbsp; | &nbsp; | unmet, and parked rather than filed because the value is marginal. Six directories build their own TAGS independently -- c/, d/, e/, m2/, system/ and html-check-links/ -- and m2/ additionally hand-rolls a TAGS.doc for the Macaulay2Doc sources by piping find into a shell loop (m2/Makefile.in:43-47). There is no etags --include anywhere in the tree and no umbrella TAGS target, so the specific mechanism the file proposes is unused and 'better tag files for everything' does not exist. Weighed against that: the m2 TAGS rule is actively maintained, #4233 having fixed an argument-list-too-long failure in it, and a developer wanting cross-directory navigation today would more likely reach for an LSP than for nested etags files. Left with disposition blank so bin/file-issues skips it and it rests at Ready |
