...
/* sets result = a sample from the standard normal distribution, rounded */
/* to the nearest rational number with denominator bounded by height */
{
  ...
  mpfr_nrandom(x, state, MPFR_RNDN);
  rawSetFareyApproximation(result, x, height);
  mpfr_clear(x);
}

gmp_QQ rawRandomQQ(gmp_ZZ height)
/* returns a sample from the standard normal distribution, rounded */
/* to the nearest rational number with denominator bounded by height */
{
  mpq_ptr result = getmemstructtype(mpq_ptr);
  mpq_init(result);
