...
doc ///
  ...
  Description
    ...
    Example
      apply(-3..3, i -> try 1/i)
    Text
      In the alternate form @M2CODE "try x except y do z"@, @VAR "x"@ is first
      evaluated. If no error occurs, the resulting value is returned. If an
      error occurs, then the corresponding @TO Error@ object is assigned to the
      symbol specified by @VAR "y"@. That symbol is then available during the
      evaluation of @VAR "z"@, whose value is returned.
    Example
